...
# Use all available data for the 1536 dataset
filtered_data_magnetic = compass_data.copy()
filtered_data_gyro = gyro_data.copy()

# Merge ground truth data with compass data to get true headings
print("Processing ground truth headings...")
...
compass_headings_raw = filtered_data_magnetic['value_2']  # Compass heading in value_2
steps = filtered_data_magnetic['step']
floors = filtered_data_magnetic['value_4']
eastings = filtered_data_magnetic['value_1']  # Easting in value_1
northings = filtered_data_magnetic['value_3']  # Northing in value_3

# Normalize compass headings to 0-360 degree range
compass_headings = [(heading + 360) % 360 for heading in compass_headings_raw]
print(f"Normalized compass headings to 0-360 degree range")
...
print(f"Detecting QS intervals with stability_threshold={stability_threshold}, window_size={window_size}...")
...
# Initialize data structure for storing QS intervals
data_QS = {
...
for timestamp, heading, true, step, floor, east, north in zip(timestamps, compass_headings, true_headings, steps, floors, eastings, northings):
...
print(f"Number of detected quasi-static intervals: {num_quasi_static_intervals}")
...
print("Calculating QS interval statistics...")
...
print(f"Average of Abs_Difference: {average_abs_difference:.2f}")
...
if num_quasi_static_intervals > 0:
    cmap = plt.cm.get_cmap('Set1', num_quasi_static_intervals)
...
# 2. Steps with QS intervals
plt.figure(figsize=(12, 6))
plt.plot(timestamps, steps, label='Steps', color='cyan', alpha=0.6)
...
# 4. 3D plot of locations with floor info
...
# 5. Floor plot with QS intervals
plt.figure(figsize=(12, 6))
...
# Extract the table of QS intervals
